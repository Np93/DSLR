...
def compare_correlation_functions(dataset, numeric_cols) -> any:
    # Clean the dataset (assuming the preprocess function removes NaNs)
    dataset_clean, valid_numeric_cols = preprocess_data_for_correlation(dataset, numeric_cols)

    # Create two DataFrames: one for NumPy results, one for manual results
    numpy_correlations = pd.DataFrame(index=valid_numeric_cols, columns=valid_numeric_cols)
    manual_correlations = pd.DataFrame(index=valid_numeric_cols, columns=valid_numeric_cols)

    # Compare the correlation values between both methods
    for col1 in valid_numeric_cols:
        for col2 in valid_numeric_cols:
            if col1 != col2:
                # NumPy's corrcoef
                numpy_corr = np.corrcoef(dataset_clean[col1], dataset_clean[col2])[0, 1]
                numpy_correlations.at[col1, col2] = numpy_corr if np.isfinite(numpy_corr) else np.nan

                # Manual Pearson correlation
                manual_corr = Pearson_correlation_coefficient(dataset_clean[col1], dataset_clean[col2])
                manual_correlations.at[col1, col2] = manual_corr if np.isfinite(manual_corr) else np.nan

    # Print both results to compare
    print("NumPy Correlations:")
    print(numpy_correlations)

    print("\nManual Correlations:")
    print(manual_correlations)

    # Optionally, return both DataFrames
    return numpy_correlations, manual_correlations
...
def main():
    ...
    """ # testing the correalation computation
    numeric_cols = ['Arithmancy', 'Astronomy', 'Herbology']
    compare_correlation_functions(dataset, numeric_cols) """
